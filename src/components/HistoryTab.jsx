 ...
 * -----------------------------------------------------------------------
 * Log of past AI searches. "Repeat" re-populates the search form fields
 * and jumps back to the Search tab so the user can run it again.
 */
import React from "react";

export default function HistoryTab({ searchHistory, onRepeatSearch }) {
  return (
    <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {searchHistory.map((h, i) => (
            ...
            >
              ...
              <button className="btn btn-s" style={{ fontSize: 12 }} onClick={() => onRepeatSearch(h)}>
                🔄 Repeat
              </button>
            </div>
          ))}
